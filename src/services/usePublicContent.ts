...
import {
  ...
  fetchPublicMeshNetwork,
  fetchPublicAssociatedWith,
  fetchPublicVisitorsList,
  resolveSafeMediaUrl,
  fetchPageVisibility,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const normalizeKey = (k: string) =>
        String(k ?? "").toLowerCase().replace(/[\s-_]+/g, "").trim();

      // Build a lookup from DB data by startup name (normalized)
      const dbByName = new Map(
        (data || []).map((d: any) => [normalizeKey(d.startup), d])
      );
      // For each of the 5 fixed slots, use DB version if available, else static default
      return (staticPortfolio as any[]).map((s: any) => {
        const slotKey = normalizeKey(s.startup);
        const match = dbByName.get(slotKey);
        const _slotId = slotKey;
        ...
        if (!match) {
          return {
            ...
            founderImage: resolveSafeMediaUrl(s.founderImage),
          };
        }
        ...
        if (match.logo) merged.logo = resolveSafeMediaUrl(match.logo);
        if (match.founder) merged.founder = match.founder;
        if (match.founderTitle) merged.founderTitle = match.founderTitle;
        if (match.founderImage) merged.founderImage = resolveSafeMediaUrl(match.founderImage);
        ...
        if (match.achievements && Array.isArray(match.achievements) && match.achievements.length > 0) {
          merged.achievements = match.achievements;
        }
        return merged;
      });
    },
    ...
    placeholderData: staticPortfolio.map(s => ({
      ...s,
      _slotId: String(s.startup ?? "").toLowerCase().replace(/[\s-_]+/g, "").trim(),
      logo: resolveSafeMediaUrl(s.logo),
      founderImage: resolveSafeMediaUrl(s.founderImage),
    })),
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const mergeDbMatch = (base: any, match: any) => {
        ...
        if (match.stats) merged.stats = match.stats;
        if (match.url) merged.url = match.url;
        return merged;
      };
      ...
      const customItems = (data || [])
        ...
        .map((d: any) => ({
          ...
          long: d.long ?? "",
          blurb: d.blurb ?? "",
          url: d.url ?? "/ecosystem/infrastructure",
          image: d.image ?? "",
          card: d.card ?? d.image ?? "",
          stats: d.stats ?? [],
        }));
...
export function usePublicPartners<T>(fallbackData: T) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (admin?.length) {
        return admin.map((url) => resolveSafeMediaUrl(url) || url) as any;
      }
      return fallbackData;
    },
    staleTime: 60_000,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (Array.isArray(data) && data.length > 0) {
        const filtered = data.filter((x) => typeof x === "string" && x.trim().length > 0);
...
export const usePublicInfrastructure = <T extends Record<string, unknown>>(fallbackData: T) =>
  useGenericPublicContent("infrastructure", async () => {
    ...
    if (stored?.hero) {
      ...
      const sanitizedStored: Record<string, string> = {};
      for (const [k, v] of Object.entries(stored)) {
        sanitizedStored[k] = resolveSafeMediaUrl(v) || v;
      }
      merged[imgKey] = { ...currentImages, ...sanitizedStored };
      return merged as T;
    }
    return fallbackData;
  }, fallbackData);
...
export const usePublicVisionRoadmap = <T extends Record<string, any>>(fallbackData: T) =>
  useGenericPublicContent("visionRoadmap", async () => {
    const data = await fetchPublicVisionRoadmap();
    if (!data) return fallbackData;
    return {
      visionData: data.visionData && data.visionData.length > 0 ? data.visionData : fallbackData.visionData,
      roadmapData: data.roadmapData && data.roadmapData.image ? data.roadmapData : fallbackData.roadmapData,
      timelineYears: data.timelineYears && data.timelineYears.length > 0 ? data.timelineYears : fallbackData.timelineYears,
    } as T;
  }, fallbackData);
...
export const usePublicDistinguishedVisitors = <T>(fallbackData: T) =>
  useGenericPublicContent("distinguishedVisitors", async () => {
    const data = await fetchPublicVisitorsList();
...
export function usePublicMeshNetwork<T>(fallbackData: T) {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await fetchPublicMeshNetwork();
      if (Array.isArray(data) && data.length > 0) return data as any;
      const admin = getAdminLocalStorage<any[]>(ADMIN_MESH_NETWORK_KEY);
      if (Array.isArray(admin) && admin.length > 0) {
        return admin.map(a => resolveSafeMediaUrl(typeof a === 'string' ? a : (a.logo_url || a.image_url || a.image || ''))) as any;
      }
      return fallbackData;
    },
    staleTime: 60_000,
...
export function usePublicAssociatedWith<T>(fallbackData: T) {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await fetchPublicAssociatedWith();
      if (Array.isArray(data) && data.length > 0) return data as any;
