...
export function resolveSafeMediaUrl(path: string | null | undefined): string {
  if (!path) return "";
  return resolveBackendMediaUrl(String(path));
}
...
export function mapIncubatorFromApi(item: Record<string, unknown>) {
  ...
  let url = item.url ? String(item.url) : item.link ? String(item.link) : "";
  ...
  if (item.description && typeof item.description === "string") {
    try {
      ...
      if (parsed.link && !url) url = String(parsed.link);
...
export async function fetchPublicMeshNetwork() {
  const data = await fetchPublic<Record<string, unknown>[]>("/api/public/mesh-network");
  if (!data?.length) return null;
  return data.map(d => resolveSafeMediaUrl(String(d.logo_url ?? d.logo ?? "")));
}

export async function fetchPublicAssociatedWith() {
  const data = await fetchPublic<Record<string, unknown>[]>("/api/public/associated-with");
  if (!data?.length) return null;
  return data.map(d => resolveSafeMediaUrl(String(d.logo_url ?? d.logo ?? "")));
}

export async function fetchPublicVisitorsList() {
  const data = await fetchPublic<Record<string, unknown>[]>("/api/public/visitors");
  if (!data?.length) return null;
  return data.map(d => ({
    name: String(d.name ?? ""),
    role: String(d.role ?? ""),
    org: String(d.org ?? ""),
    image: resolveSafeMediaUrl(String(d.image_url ?? d.image ?? "")),
  }));
}
...
export function mapStartupEventFromApi(item: Record<string, unknown>) {
  return {
    ...
    images: Array.isArray(item.images)
      ? item.images.map((img: string) => resolveSafeMediaUrl(img))
      : (item.image_url_1 || item.image_url_2 || item.image_url_3)
      ? [
          resolveSafeMediaUrl(String(item.image_url_1 || item.image_url || "")),
          resolveSafeMediaUrl(String(item.image_url_2 || "")),
          resolveSafeMediaUrl(String(item.image_url_3 || ""))
        ].filter(Boolean)
      : typeof item.images === "string"
      ? (() => {
          try {
            const parsed = JSON.parse(item.images as string);
            return Array.isArray(parsed) ? parsed.map((img: string) => resolveSafeMediaUrl(img)) : undefined;
          } catch {
            return undefined;
          }
        })()
...
export const fetchPublicInfrastructure = async () => {
  const S3_AHUB = "https://dtfhihp4ovn79.cloudfront.net/Infrastructure/ahub.jpg";
  const S3_COLLAB = "https://dtfhihp4ovn79.cloudfront.net/Infrastructure/collabrative+environment.png";
  const S3_CONF = "https://dtfhihp4ovn79.cloudfront.net/Infrastructure/confenrce+hall.jpg";
  const S3_IOT = "https://dtfhihp4ovn79.cloudfront.net/Infrastructure/iot+labs.jpeg";

  const defaultImages = {
    hero: S3_AHUB,
    collaborative: S3_COLLAB,
    conference: S3_CONF,
    labs: S3_IOT,
    campus: S3_AHUB,
    galleryCoworking: S3_AHUB,
    galleryStartupBays: S3_COLLAB,
    galleryEventHall: S3_CONF,
    gallerySeminarRoom: S3_COLLAB,
    galleryInnovationLabs: S3_IOT,
    masonryCampus: S3_AHUB,
    masonryIoT: S3_IOT,
    masonryCollaborative: S3_COLLAB,
    masonryConference: S3_CONF,
    masonryWorkspace: S3_AHUB,
    masonryResearch: S3_IOT,
  };
  ...
  if (!items?.length) return { infrastructureImages: defaultImages };

  // Build a lookup by (section, label) -> resolved image URL
  const lookup = new Map<string, string>();
  for (const item of items) {
    const section = String(item.section ?? "").toLowerCase().trim();
    const label = String(item.label ?? "").toLowerCase().trim();
    let imageUrl = resolveSafeMediaUrl(String(item.image_url ?? ""));
    if (imageUrl.includes("localhost:8000") || !imageUrl) {
      if (label.includes("collab") || label.includes("bays") || label.includes("seminar")) imageUrl = S3_COLLAB;
      else if (label.includes("conf") || label.includes("event")) imageUrl = S3_CONF;
      else if (label.includes("iot") || label.includes("lab") || label.includes("research")) imageUrl = S3_IOT;
      else imageUrl = S3_AHUB;
    }
  ...
  const get = (section: string, label: string, fallback: string) =>
    lookup.get(`${section.toLowerCase()}::${label.toLowerCase()}`) || fallback;

  // Map to the infrastructureImages shape the frontend page expects
  const infrastructureImages = {
    hero: get("hero", "hero image", S3_AHUB),
    collaborative: get("side", "collaborative environment", S3_COLLAB),
    conference: get("side", "events & community spaces", S3_CONF),
    labs: get("side", "advanced research facilities", S3_IOT),
    campus: get("side", "facility gallery", S3_AHUB),
    galleryCoworking: get("gallery", "coworking space", S3_AHUB),
    galleryStartupBays: get("gallery", "startup bays", S3_COLLAB),
    galleryEventHall: get("gallery", "event hall", S3_CONF),
    gallerySeminarRoom: get("gallery", "seminar room", S3_COLLAB),
    galleryInnovationLabs: get("gallery", "innovation labs", S3_IOT),
    masonryCampus: get("masonry", "ahub campus", S3_AHUB),
    masonryIoT: get("masonry", "iot & robotics lab", S3_IOT),
    masonryCollaborative: get("masonry", "collaborative zones", S3_COLLAB),
    masonryConference: get("masonry", "conference hall", S3_CONF),
    masonryWorkspace: get("masonry", "founder workspace", S3_AHUB),
    masonryResearch: get("masonry", "research lab", S3_IOT),
  };
...
export const fetchPublicVisionRoadmap = async () => {
  const S3_VISION = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/vision.jpg";
  const S3_MISSION = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/mission.jpeg";
  const S3_ROADMAP = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/roadmap.png";
  const S3_2021 = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/2021-2022.png";
  const S3_2022 = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/2022-2023.png";
  const S3_2023 = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/2023-2024.png";

  const data = await fetchPublic<Record<string, unknown>>(PUBLIC_CONTENT_ENDPOINTS.visionRoadmap);
  if (!data) return null;
  const rawVision = Array.isArray(data.visionData) ? data.visionData : [];
  const rawTimeline = Array.isArray(data.timelineYears) ? data.timelineYears : [];
  const rawRoadmap = data.roadmapData as any;
  if (!rawVision.length && !rawTimeline.length && (!rawRoadmap || !rawRoadmap.image)) return null;
  return {
    visionData: rawVision.length ? rawVision.map((v: any) => {
      let image = resolveSafeMediaUrl(v.image) || v.image;
      if (!image || image.includes("localhost:8000")) {
        const isMission = (v.id === "vision-02") || String(v.title || "").toLowerCase().includes("mission") || String(v.heading || "").toLowerCase().includes("mission");
        image = isMission ? S3_MISSION : S3_VISION;
      }
      return { ...v, image };
    }) : undefined,
    roadmapData: rawRoadmap && (rawRoadmap.image || rawRoadmap.title) ? {
      ...rawRoadmap,
      image: (resolveSafeMediaUrl(rawRoadmap.image) || rawRoadmap.image)?.includes("localhost:8000") ? S3_ROADMAP : (resolveSafeMediaUrl(rawRoadmap.image) || S3_ROADMAP),
    } : undefined,
    timelineYears: rawTimeline.length ? rawTimeline.map((t: any) => {
      let image = resolveSafeMediaUrl(t.image) || t.image;
      if (!image || image.includes("localhost:8000")) {
        const yr = String(t.year || "");
        if (yr.includes("2021")) image = S3_2021;
        else if (yr.includes("2022")) image = S3_2022;
        else image = S3_2023;
      }
      return { ...t, image };
    }) : undefined,
