                  ...
                  <div key={event.id} className="flex items-center gap-3 rounded-xl bg-white p-3 shadow-sm">
                    {company?.logo ? (
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#FFF4E8] p-1">
                        <img loading="lazy" decoding="async" src={company.logo} alt="" className="max-h-full max-w-full object-contain" />
                      </div>
                    ) : null}
...
function StartupLogoBanner({ logo, name, size = "md" }: { logo: string; name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    ...
    >
      ...
      <img loading="lazy" decoding="async" src={logo} alt={name} className={cn("relative z-10 object-contain", logoSizes[size], size === "lg" ? "max-w-[180px]" : "max-w-[120px]")} />
    </div>
  );
}
