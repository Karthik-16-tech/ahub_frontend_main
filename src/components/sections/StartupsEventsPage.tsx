    ...
    return [
      {
        ...
        image: "https://dtfhihp4ovn79.cloudfront.net/startup_events/women_tie.jpeg",
        type: "Upcoming Event",
      ...
      },
      {
        ...
        image: "https://dtfhihp4ovn79.cloudfront.net/startup_events/startup+starurdays.jpg",
        type: "Networking",
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          {event.partners.map((p) => (
                            <div key={p.name} title={p.name} className="flex items-center justify-center h-9 w-16 rounded-lg overflow-hidden bg-white border border-[#F5E6D3] shadow-md">
                              <img loading="lazy" decoding="async" src={p.logo} alt={p.name} className="h-full w-full object-contain p-1" />
                            </div>
                          ))}
                        </div>
                      )}
  ...
  if (isUrl(logoType)) {
    return (
      <div className="absolute bottom-3 left-3 flex h-12 w-12 items-center justify-center rounded-xl bg-white shadow-md border border-slate-100 overflow-hidden">
        <img loading="lazy" decoding="async" src={logoType} alt="" className="h-8 w-8 object-contain" />
      </div>
    );
  }
  switch (logoType) {
