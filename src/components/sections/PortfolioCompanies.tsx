      ...
      <div className="site-container-wide">
        ...
        <div className="hidden md:flex mt-20 h-[450px] gap-3 md:h-[480px]">
          {portfolio.map((p: any, i: number) => {
            ...
            const logoUrl = resolveBackendMediaUrl(p.logo);
            const founderImageUrl = resolveBackendMediaUrl(p.founderImage);
            const startupName = p.startup || "Startup";
            const founderName = p.founder || "";
              ...
              >
                ...
                <div className={`absolute inset-0 flex items-end justify-center pb-12 transition-all duration-500 ${isActive ? "opacity-0 scale-95" : "opacity-100 scale-100"
                  ...
                  {logoUrl ? (
                    ...
                    >
                      <span className="-rotate-90 block tracking-wider select-none">{startupName.charAt(0)}</span>
                    </div>
                  )}
                </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="absolute left-[-32px] md:left-[-40px] top-[48px] bg-[#FF7A00] p-[3px] pr-[4px] shadow-[0_4px_12px_rgba(255,122,0,0.12)] z-20"
                        ...
                        <div className="bg-white h-[60px] flex items-center pl-9 md:pl-11 pr-9"
                          ...
                          {logoUrl ? (
                            ...
                            <span className="font-display font-black text-lg text-[#0F172A] whitespace-nowrap pr-2">{startupName}</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-[86px] space-y-2">
                        <h3 className="font-display text-[1.2rem] font-bold tracking-tight text-[#0F172A] leading-[1.2] sm:text-[1.32rem]">
                          Your Partner in <br />
                          <span className="text-[#0F172A]">{startupName}</span> <span className="text-[#FF7A00]">Success</span>
                        </h3>
                      ...
                      <div className="flex items-center gap-6 mt-4 border-t border-slate-100 pt-4">
                        {getStatsForStartup(startupName, p.achievements || []).map((stat, index, arr) => {
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:hidden">
          {portfolio.map((p: any, idx: number) => {
            ...
            const logoUrl = resolveBackendMediaUrl(p.logo);
            const founderImageUrl = resolveBackendMediaUrl(p.founderImage);
            const achievementsList = Array.isArray(p.achievements) ? p.achievements : ["AI Mock Interviews", "Placement Ready", "Career Growth"];
            const startupName = p.startup || "Startup";
            const founderName = p.founder || "";
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-1">
                  {achievementsList.slice(0, 2).map((achievement: string, aIdx: number) => {
                ...
                <div className="flex items-center gap-3 border-t border-orange-100/30 pt-3.5 mt-1">
                  ...
                  <div>
                    ...
                    <h4 className="text-[11px] font-bold text-[#1C1412] tracking-tight mt-0.5 leading-none font-display">{founderName}</h4>
                    <p className="text-[8px] font-medium text-[#6C5E5B]/75 leading-none mt-1">{p.founderTitle || "Founder & CEO"}</p>
                  </div>
                </div>
              </div>
            );
