...
function VisionCards({ data }: { data: any }) {
  ...
  return (
    <section className="bg-[#FFF8F2] px-4 pb-12 sm:pb-16">
      <div className="site-container-wide">
        <div className="grid gap-6 md:grid-cols-2">
          {cards.map((item: any) => {
            const Icon = item.icon === "Rocket" ? Rocket : Target;
            const imgSrc = resolveBackendMediaUrl(item.image);
            return (
              <div
              ...
              >
                <div className="grid md:grid-cols-2">
                  {/* Image */}
                  <div className="relative h-48 overflow-hidden md:h-full bg-slate-100">
                    {imgSrc ? (
                      <img
                        src={imgSrc}
                      ...
                      <div className="flex h-full w-full items-center justify-center bg-orange-50 text-[#FF6B00]">
...
function RoadmapSection({ data }: { data: any }) {
  const fallbackUrl = "https://dtfhihp4ovn79.cloudfront.net/vision_and+_roadmap/roadmap.png";
  const roadmapImg = resolveBackendMediaUrl(data?.image || fallbackUrl) || fallbackUrl;
  const title = data?.title || "Strategic Roadmap";

  return (
    <section className="bg-[#FAF7F4] px-4 py-12 sm:py-16">
      <div className="site-container-wide">
        <div className="text-center">
          <p className="text-xs font-bold uppercase tracking-[0.25em] text-[#FF6B00]">
            Our Roadmap
          </p>
          <h2 className="mt-2 text-2xl font-bold tracking-tight text-[#0B1736] sm:text-3xl md:text-4xl">
            {title}
          </h2>
          <div className="mx-auto mt-3 h-px w-12 bg-[#FF6B00]" />
        </div>

        <div className="mx-auto mt-8 sm:mt-10 max-w-5xl">
          <div className="overflow-hidden rounded-2xl shadow-xl shadow-[#FF6B00]/10 bg-white p-2 sm:p-4 border border-orange-100/50 min-h-[160px] flex items-center justify-center">
            <div className="w-full overflow-x-auto">
              <img
                ...
                className="w-full min-w-[280px] h-auto object-contain block mx-auto rounded-xl"
              />
            </div>
          </div>
        </div>
              ...
              >
                <div className="relative h-44 overflow-hidden bg-slate-100">
                  {imgSrc ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-orange-50 text-[#FF6B00] font-bold">
                      {item.year}
                    </div>
                  )}
...
export function VisionRoadmapPage() {
  ...
  return (
    <>
      <HeroSection />
      <VisionCards data={data?.visionData || visionData} />
      <RoadmapSection data={data?.roadmapData || roadmapData} />
      <TimelineSection data={data?.timelineYears || timelineYears} />
    </>
  );
}
