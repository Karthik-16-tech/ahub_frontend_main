...
export function InternshipRegistrationPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(`${base}/api/public/join-us/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          form_type: "internship_registration",
          data: {
            ...
            statement_of_purpose: formData.motivation,
            resume_filename: resumeName || "Uploaded via form",
          },
        }),
      });
    } catch (err) {
              ...
              <div className="flex items-center justify-between border-b border-slate-100 bg-[#FDF8F2] px-6 py-4">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white p-2 shadow-sm border border-slate-100">
                    <img loading="lazy" decoding="async" src={selectedInternship.logo} alt="Logo" className="max-h-full max-w-full object-contain" />
                  </div>
                  <div>
